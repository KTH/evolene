...
class FromImageStep(AbstractPipelineStep):
    ...
    def run_step(self, data):
        ...
        if self.validate(from_line, data):
            ...
            ci_status.post_platform_validation_run(data, ci_status.STATUS_OK, 0)
            self.step_ok()
        else:
            ...
            self.step_warning()
            slack.send(text)
            ci_status.post_platform_validation_run(data, ci_status.STATUS_ERROR, 7, 'Dockerfile based on old image')
